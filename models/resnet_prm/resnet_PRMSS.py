...
__all__ = ['prmss__16x1_l1norm_resnet50','prmss__4x16_l1norm_resnet50']
...
class PointwiseGroupConv(nn.Module):
    def __init__(self,inchannel, outchannel, groups):
        super(PointwiseGroupConv, self).__init__()
        self.parameter = Parameter(torch.rand(1,groups,inchannel//groups,outchannel//groups,1,1))
        self.groups = groups
    def forward(self, x):
        b,c,h,w = x.size()
        x = x.view(b,self.groups,c//self.groups,1,h,w)
        x = (x*self.parameter).sum(dim=2)
        return x.view(b,-1,w,h)
...
class PRMLayer(nn.Module):
    def __init__(self, channel,reduction=16,groups=4,mode='l1norm'):
        ...
        self.query = PointwiseGroupConv(channel,channel//reduction,self.groups)
        self.key = PointwiseGroupConv(channel, channel // reduction, self.groups)
...
def prmss__16x1_l1norm_resnet50(pretrained=False, **kwargs):
...
def prmss__8x8_l1norm_resnet50(pretrained=False, **kwargs):
    """Constructs a ResNet-50 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
...
def prmss__4x16_l1norm_resnet50(pretrained=False, **kwargs):
    """Constructs a ResNet-50 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
...
def demo():
    ...
    for i in range(1):
        net = prmss__16x1_l1norm_resnet50(num_classes=1000)
...
def demo2():
    st = time.perf_counter()
    for i in range(1):
        net = prmss__8x8_l1norm_resnet50(num_classes=1000).cuda()
