...
__all__ = ['prm4_resnet18','prm4_resnet34','prm4_resnet50','prm4_resnet101','prm4_resnet152']

"""
group is the number of selected points.
"""
class PRMLayer(nn.Module):
    def __init__(self, channel,reduction=4,groups=16,mode='l1norm'):
    ...
    def forward(self, x):
        ...
        similarity = similarity.view(b * self.groups, 1, h, w)

        value = (x.view(b * self.groups, -1, h, w))*(self.sig(similarity))
...
def prm4_resnet18(pretrained=False, **kwargs):
...
def prm4_resnet34(pretrained=False, **kwargs):
...
def prm4_resnet50(pretrained=False, **kwargs):
...
def prm4_resnet101(pretrained=False, **kwargs):
...
def prm4_resnet152(pretrained=False, **kwargs):
...
def demo():
    ...
    for i in range(1):
        net = prm4_resnet50(num_classes=1000)
...
def demo2():
    st = time.perf_counter()
    for i in range(1):
        net = prm4_resnet50(num_classes=1000).cuda()
