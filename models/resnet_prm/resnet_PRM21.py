...
__all__ = ['prm21_resnet18','prm21_resnet34','prm21_resnet50','prm21_resnet101','prm21_resnet152']
...
class PRMLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        b,c,h,w = x.size()
        # Similarity function
        query_value, _ = self.get_query_position(x, self.groups)  # shape [b*num,2,1,1]
...
def prm21_resnet18(pretrained=False, **kwargs):
...
def prm21_resnet34(pretrained=False, **kwargs):
...
def prm21_resnet50(pretrained=False, **kwargs):
...
def prm21_resnet101(pretrained=False, **kwargs):
...
def prm21_resnet152(pretrained=False, **kwargs):
...
def demo():
    ...
    for i in range(1):
        net = prm21_resnet50(num_classes=1000)
...
def demo2():
    st = time.perf_counter()
    for i in range(1):
        net = prm21_resnet50(num_classes=1000).cuda()
