...
def prm_16x4_l1norm_resnet50(pretrained=False, **kwargs):
...
def prm_16x4_dotproduct_resnet50(pretrained=False, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(Bottleneck, [3, 4, 6, 3],prm_r=16,prm_g=4,prm_mode='dotproduct', **kwargs)
    return model

def prm_16x1_dotproduct_resnet50(pretrained=False, **kwargs):
    """Constructs a ResNet-50 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(Bottleneck, [3, 4, 6, 3],prm_r=16,prm_g=1,prm_mode='dotproduct', **kwargs)
    return model
...
def demo():
    ...
    for i in range(1):
        net = prm_16x4_dotproduct_resnet50(num_classes=1000)
...
def demo2():
    st = time.perf_counter()
    for i in range(1):
        net = prm_16x1_dotproduct_resnet50(num_classes=1000).cuda()
